...
const CARD_CENTER_NUDGE_Y_MOBILE = 0.9;
...
function KeyboardNavigator({ planes }: { planes: number }) {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      // only handle when an image is expanded
      if ((ctx?.expandedIndexRef?.current ?? null) == null) return;
...
function ElasticKnob({
  ...
  return (
    <div
      ref={hostRef}
      className="axon-knob sticky-bottom"
